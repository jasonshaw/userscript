// ==UserScript==
// @name                专用链破解 Special Links Converter 去后门版
...
// @include             http://hkbici.com/forum.php?mod=viewthread&*
// @include             http://hkbici.com/thread-*-1-*.html
// @include             http://184.171.166.59/forum.php?mod=viewthread&*
// @include             http://184.171.166.59/thread-*-1-*.html
// @include             http://upstream.hkbici.com/forum.php?mod=viewthread&*
// @include             http://upstream.hkbici.com/thread-*-1-*.html
// @include             http://*hkbici.net/forum.php?mod=viewthread&*
// @include             http://*hkbici.net/thread-*-1-*.html
// @grant               GM_getValue
// @grant               GM_setValue
// @grant               GM_addStyle
...
(function(){
	...
	GM_getValue('_selecLink',true) && document.addEventListener('mouseup', function(e){
		...
		if (e.button == 0 && r.test(selection)){
			...
			!/^http|^ftp/i.test(linkCode) && (linkCode = (linkCode.indexOf('ed2k://') == 0 ?linkCode : 'http://' + linkCode));
			e.target.innerHTML = e.target.innerHTML.replace(/amp;/g, '').replace(selection.match(r)[0], linkCode.link(linkCode));
		}
		GM_setClipboard(linkCode);
	}, false);
