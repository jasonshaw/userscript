...
function renameChain(){
	...
	var radios = document.getElementsByName("radiobox"),radioChecked = null,i;
	for(i = 0;i < radios.length;i++) { if(radios[i].checked) { radioChecked = radios[i].value;break;}}
	var remove = document.getElementById("_remove").checked == true? true : false;
	...
	switch(radioChecked){
		case "_plus":
			if(remove) {
				if(insertBefore == ''&& insertAfter == '') {input.value = fname.replace(/\(\d+\)/ig,'');}//alert(fname);
				else input.value = fname.replace(insertBefore,'').replace(insertAfter,'');
			}
			else {
